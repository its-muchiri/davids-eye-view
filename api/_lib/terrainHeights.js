...
/**
 * Terrain-heights proxy handler. Shared verbatim between the Vite dev-server
 * plugin (vite.config.js: terrainHeightsProxy) and the Vercel Function
 * entrypoint (api/terrain/heights.js). Core parsing/resolution logic already
 * lived in src/data/terrainHeightsProxy.js before this port — only the
 * request-handling glue (caching, disk persistence) moved here.
 *
 * Disk persistence to `.gev-cache/` is best-effort, same as celestrak.js:
 * writeback failures are caught and logged, never thrown, so a read-only
 * filesystem (Vercel outside /tmp) degrades to memory-only caching within
 * the warm instance rather than failing requests.
 */
...
export default async function terrainHeightsHandler(req, res) {

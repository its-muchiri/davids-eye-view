...
// v2 perf fix: the catalog spans every source pack at once (Austin +
// Caltrans + TfL London, up to CCTV_MAX_SOURCES), so an unscoped drain
// pays GEO_LOAD_BATCH_SIZE/GEO_LOAD_BATCH_DELAY_MS pacing for cameras the
// viewer has no chance of seeing (measured: ~20s to drain 900 records at
// 4/120ms). Only records within this radius of the enable-time viewer
// position count toward the tracked/blocking initial drain (same 250km
// "nearby" convention the app already uses elsewhere, e.g. Contacts); the
// rest resolve afterward via the existing untracked background path
// (enqueueGeometryRefresh) so the loading chip reflects reality and
// finishes in proportion to what's actually near the camera.
const GEO_LOAD_NEARBY_RADIUS_KM = 250;
...
// Records outside GEO_LOAD_NEARBY_RADIUS_KM at enable time — deferred out of
// the tracked initial drain, then handed to the untracked background queue
// once the nearby set finishes (see startGeometryLoadQueue / processGeometryBatch).
let _geoDeferredFar = [];
  ...
  const batchResult = processCctvGeometryDrainBatch({
    ...
    complete: () => {
      ...
      if (wasInitialLoad) {
        ...
        // The nearby/tracked drain just finished — hand off any out-of-radius
        // records (GEO_LOAD_NEARBY_RADIUS_KM) to the untracked background
        // queue now, so the loading chip has already reported done and this
        // continuation never re-arms _geoLoading.
        if (_geoDeferredFar.length) {
          const deferred = _geoDeferredFar;
          _geoDeferredFar = [];
          enqueueGeometryRefresh(deferred);
        }
      }
...
function startGeometryLoadQueue() {
  ...
  const pending = sorted
    .filter((entry) => entry.distKm <= GEO_LOAD_NEARBY_RADIUS_KM)
    .map((entry) => entry.record);
  // Out-of-radius records (e.g. a different source pack's city entirely)
  // never join the tracked/blocking drain — they resolve later, silently,
  // once the nearby set completes (see processGeometryBatch's complete()).
  _geoDeferredFar = sorted
    .filter((entry) => entry.distKm > GEO_LOAD_NEARBY_RADIUS_KM)
    .map((entry) => entry.record);
